...
export const fetchPrice = async data => {
  ...
  const response = await fetch(`${pricingPage}/${value}`, { next: { revalidate: 21600 } })
...
export const fetchAllPrice = async info => {
  ...
  const regionalPricing = await kv.get('regional-pricing')
  if (regionalPricing) {
  ...
  // Update KV
  kv.put('regional-pricing', JSON.stringify(regionalPricingData), {
